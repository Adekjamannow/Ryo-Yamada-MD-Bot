...
let handler = async (m, { conn, usedPrefix, command, text }) => {
  try {
    ...
    const apiUrl = `https://api.nekolabs.my.id/tools/pxpic/removebg?imageUrl=${encodeURIComponent(url)}`

    const resApi = await fetch(apiUrl)
    if (!resApi.ok) throw '❌ Gagal menghubungi server.'
    const json = await resApi.json()
    if (!json.status) throw '❌ Gagal menghapus background.'

    const hasil = json.result
    await conn.sendMessage(m.chat, { image: { url: hasil }, caption: "✨ Background berhasil dihapus (v2)!" }, { quoted: m })
  } catch (e) {
    console.error(e)
